...
def _fingerprint(event: Dict[str, Any]) -> str:
    article_ids = sorted(
        str(item.get("id")) for item in event.get("evidence_articles", []) if item.get("id")
    )
    return hashlib.sha256("\x1f".join(article_ids).encode("utf-8")).hexdigest()


def _severity(event: Dict[str, Any]) -> str:
    evidence_count = len(event.get("evidence_articles") or [])
    ...
    try:
        ...
        for event in events:
            ...
            payload = {
                ...
                "signal_reasons": event.get("signal_reasons") or [],
